 ...
 try{
  ...
  await page.evaluate(catalog=>{
   ...
   window.api=async(name,...args)=>{
    ...
    if(name==='bulk_tweaks')return{ok:true,applied:{bing_search:true,gamedvr:false}};
    if(name==='nvprofile_status')return{nvidia:false};
    return null;
   };
   STATE.tweaks=[{key:'bing_search',category:'Explorer & UI',applied:false},{key:'gamedvr',category:'Performance',applied:false}];
   STATE.cats=['Explorer & UI','Performance'];buildNav();wireBulk();show('Debloat & Customization');
  },catalog);
